$(function() {
                ...
                if (data.length == 0) {
                    ...
                    for (let i = 0, len = data.length; i < len; i++) {
                        for (let j = 0, lenj = goodsData.length; j < lenj; j++) {
                            if (data[i].product_id == goodsData[j].id) {
                                temp += `<dt>
                                            <input type="checkbox" price=${goodsData[j].price.replace('￥', '')} amount=${data[i].product_amount}>
                    ...
                    $('.goods_item').click((e) => {
                        if (e.target.tagName == 'INPUT') {
                            let added = parseFloat($(e.target).attr('price')) * parseInt($(e.target).attr('amount'))
                            let current = parseInt($('.cart_total_price').html())
                            if (e.target.checked) {
                                $('.cart_total_price').html(current + added)
                            } else {
                                $('.cart_total_price').html(current - added)
                            }
                        }
    ...
    $('.user_in_header').click(() => {
        window.location.href = 'login.html'
    })

    $('.nav_left span:nth-of-type(1)').click(() => {
